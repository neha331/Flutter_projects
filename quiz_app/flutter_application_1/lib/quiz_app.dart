...
class QuizController extends GetxController {
  var questionScreen = 1.obs;
  var queNum = 0.obs;
  var selectedAnswerIndex = -1.obs;
  var score = 0.obs;
  var allQuestions = List<SingleQuestionMode>.empty().obs;

  @override
  void onInit() {
    super.onInit();
    _retrieveQuestionsFromFirestore();
  }

  Future<void> _retrieveQuestionsFromFirestore() async {
    try {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('questions').get();
      List<SingleQuestionMode> questions = querySnapshot.docs.map((doc) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        return SingleQuestionMode.fromMap(data);
      }).toList();
...
class QuizApp  extends StatelessWidget {
  ...
  List<SingleQuestionMode> allQuestions = [];
...
//   }
  ...
  MaterialStateProperty<Color?> checkAnswer(int buttonIndex){
    if(quizController.selectedAnswerIndex != -1){
      if (buttonIndex == quizController.allQuestions[queNum].answerIndex){
        return const MaterialStatePropertyAll(Colors.green);
      }else if (buttonIndex == quizController.selectedAnswerIndex){
  ...
  if (quizController.selectedAnswerIndex == quizController.allQuestions[quizController.queNum.value].answerIndex) {
    quizController.score += 1;
  }
  if (quizController.selectedAnswerIndex != -1) {
    if (quizController.queNum == quizController.allQuestions.length - 1) {
      quizController.questionScreen.value = 3;
    }
    quizController.selectedAnswerIndex = -1;
    quizController.queNum += 1;
  }
}
  // void validateCurrentPage(){
  ...
  //               if(quizController.selectedAnswerIndex == -1){
  //             //   setState(() {
  //                 quizController.selectedAnswerIndex =0;
  //             //   });
  //              }
  //              if (quizController.selectedAnswerIndex== -1) {
  //                    quizController.selectedAnswerIndex = 0;
  ...
  //         onPressed:(){
  //              if(quizController.selectedAnswerIndex == -1){
  //               quizController.selectedAnswerIndex =2;
  //               // setState(() {
  //               //   quizController.selectedAnswerIndex =2;
  //               // });
  //             }
  ...
  //              if(quizController.selectedAnswerIndex == -1){
  //               // setState(() {
  //                  quizController.selectedAnswerIndex =3;
  //               // });
  //              }
  //         },
  //          child: Text(
...
Widget isQuestionScreen () {
  return Obx(() {
    if (quizController.questionScreen.value == 1) {
      return Scaffold(
        backgroundColor:const Color.fromARGB(255, 142, 70, 171),
        appBar: AppBar(
          title:const Text(
        ...
        body:Column(
          ...
          Row(
            ...
            children: [
              ...
              Text(
                "${queNum + 1}/${quizController.allQuestions.length}",
                 style: const TextStyle(
                   fontSize: 25,
                   fontWeight:FontWeight.w600,
                )
          ...
          SizedBox(
            ...
            height:70,
            child: Center(
              child: Text("${quizController.allQuestions[queNum].question}",
                textAlign: TextAlign.center,
                style: const TextStyle(
                 fontSize: 25,
                 fontWeight:FontWeight.w500,
          ...
          ElevatedButton(
            ...
            child: Text(
              "${quizController.allQuestions[queNum].options?[0]}",
               style:const TextStyle(
                fontSize: 20,
                fontWeight: FontWeight.normal,
              )
        ...
        ElevatedButton(
          ...
          child: Text(
            "${quizController.allQuestions[queNum].options?[1]}",
            style:const TextStyle(
              fontSize: 20,
              fontWeight: FontWeight.normal,
            )
        ...
        ElevatedButton(
          ...
          "${quizController.allQuestions[queNum].options?[2]}",
            style:const TextStyle(
              fontSize: 20,
              fontWeight: FontWeight.normal,
            )
        ...
        ElevatedButton(
          ...
          onPressed:(){
           ...
           child: Text(
            "${quizController.allQuestions[queNum].options?[3]}",
            style:const TextStyle(
              fontSize: 20,
              fontWeight: FontWeight.normal,
            )
    ...
    } else if (quizController.questionScreen.value == 2) {
